...
exports.getAllDistrict = async (req, res) => {
    try {
        ...
        let sorting = {};
        if (sort === "districtAsc") {
            sorting = { districtName: 1 };
        }
        else if (sort === "districtDesc") {
            sorting = { districtName: -1 };
        }
        ...
        if (search) {
            districts = districts.filter((a) =>
                ...
                a.stateId?.stateName.toLowerCase().includes(search.toLowerCase())
            );
        }

        if (sort === "stateAsc") {
            districts.sort((a, b) =>
                (a.stateId?.stateName || "").localeCompare(b.stateId?.stateName || "")
            );
        } else if (sort === "stateDesc") {
            districts.sort((a, b) =>
                (b.stateId?.stateName || "").localeCompare(a.stateId?.stateName || "")
            );
        }
        ...
        if (page && limit) {
            ...
            const totalPages = Math.ceil(totalDistricts / limitNum) || 1;
...
exports.downloadDistrictPdf = async (req, res) => {
    try {
        ...
        let sorting = {};
        if (sort === "districtAsc") sorting = { districtName: 1 };
        else if (sort === "districtDesc") sorting = { districtName: -1 };
